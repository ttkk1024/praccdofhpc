...
```bash
# 下载OpenMPI
# 1. Install prerequisites (compiler and tools)
sudo apt update
sudo apt install build-essential gfortran # Add gfortran for Fortran support

# 2. Create a working directory
mkdir ~/openmpi_build && cd ~/openmpi_build

# 3. Download the latest source (Check open-mpi.org for the newest version)
# Example for version 4.1.1:
...
# 6. Configure, compile, and install (installing to /opt/openmpi as an example)
./configure --prefix=/opt/openmpi
make all
sudo make install

# 7. Set up environment variables in your ~/.bashrc
